...
if ($category_id <= 0) {
    header('Location: ' . base_path('/app1/public/index.php'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['choice_id'])) {
    ...
    if ($row) {
        ...
        // フィードバック表示へ
        header('Location: ' . base_path('/app1/public/quiz.php'));
        exit;
    }
}
...
<body>
  <div class="container">
    <header class="header">
      ...
      <nav>
        <a class="button secondary" href="<?php echo e(base_path('/app1/public/index.php')); ?>">カテゴリへ戻る</a>
      </nav>
    </header>
    ...
    <?php if ($finished): ?>
      ...
      <?php endforeach; ?>
      <a class="button" href="<?php echo e(base_path('/app1/public/index.php')); ?>">トップへ</a>
    ...
    <?php else: ?>
      <div class="card">
        ...
        <h2><?php echo e($question['text']); ?></h2>
          <form method="post" class="choices" aria-label="選択肢">
            <?php echo csrf_field(); ?>
            <?php foreach ($choices as $i => $ch): ?>
              <button
                ...
                aria-label="選択肢 <?php echo $i + 1; ?>: <?php echo e($ch['text']); ?>"
              >
                <span class="choice-key"><?php echo $i + 1; ?></span>
                <span class="choice-text"><?php echo e($ch['text']); ?></span>
              </button>
            <?php endforeach; ?>
          </form>
      </div>
